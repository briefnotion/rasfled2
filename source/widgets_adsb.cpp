...
{
  // simple draw line
  Draw_List->AddLine(Screen_Position_1, Screen_Position_2, sdSysData.COLOR_SELECT.neo_color_STANDARD(Color), Size);
}
...
{
  // simple draw line
  Draw_List->AddRect(Screen_Position_1, Screen_Position_2, sdSysData.COLOR_SELECT.neo_color_STANDARD(Color), 0.0f, 0, Size);
}

void draw_marker(ImDrawList *Draw_List, system_data &sdSysData, ImVec2 Screen_Position, int Color)
{
  Draw_List->AddNgonFilled(Screen_Position, 2.0f, (ImU32)sdSysData.COLOR_SELECT.neo_color_STANDARD(Color), 4.0f);
}

void draw_marker_filled(ImDrawList *Draw_List, system_data &sdSysData, ImVec2 Screen_Position, int Color)
{
  Draw_List->AddNgonFilled(Screen_Position, 4.0f, (ImU32)sdSysData.COLOR_SELECT.neo_color_STANDARD(Color), 4.0f);
}

void draw_airport_marker(ImDrawList *Draw_List, system_data &sdSysData, ImVec2 Screen_Position, int Color)
{
  Draw_List->AddNgon(Screen_Position, 4.0f, sdSysData.COLOR_SELECT.neo_color_STANDARD(Color), 4.0f, 1.5f);
}
                ...
                ImVec2 Center_Lat_Lon, float Map_Bearing, DETAILED_TRACK &Track)
{
  ...
  for(int position = 1; position < (int)Track.TRACK_POINTS_SIMPLE.size(); position = position + Draw_Level_Of_Detail)
  {
    ...
    Draw_List->AddLine(track_position_0, track_position_1, 
                        sdSysData.COLOR_SELECT.neo_color_TEXT(RAS_GREY), 1.0f);
  }
  ...
  for(int position = 1; position < (int)Track.TRACK_POINTS_DETAILED.size(); position++)
  {
    ...
    if (draw_0 || draw_1)
    {
      ImColor point_color = sdSysData.COLOR_SELECT.neo_color_TEXT(Color_Scale.get_color(Track.TRACK_POINTS_DETAILED[position].ALTITUDE));
      ...
      Draw_List->AddLine(track_position_0, track_position_1, 
                          sdSysData.COLOR_SELECT.neo_color_TEXT(RAS_GREY), 2.0f);
    }
  }
}
                      ...
                      ImVec2 Center_Lat_Lon, float Map_Bearing, float Range)
{
  ...
  if (draw || TYPE == 3)
  {
    ...
    switch (TYPE)
    {
        ...
        if (AIRPORT_LANDING_VECTORS.size() > 1)
        {
          ...
          for(int vector = 0; vector < (int)AIRPORT_LANDING_VECTORS.size(); vector++)
          {
            ...
            Draw_List->AddLine(draw_position, landing_vector_end, 
                                sdSysData.COLOR_SELECT.neo_color_TEXT(RAS_WHITE), 2);
          }
        }
      ...
      case 2: // 2 - Region
      {
        ImColor Color = sdSysData.COLOR_SELECT.neo_color_STANDARD_V(RAS_CYAN);
      ...
      case 3: // 2 - Interstate
      {
        ImColor Color = sdSysData.COLOR_SELECT.neo_color_STANDARD(RAS_CYAN);
  ...
  if (TEST_ALTERTATIVE_COMPASS)    // Display calibration
  {
    ...
    */
    ...
    p2 = ImVec2(center.x + (sdSysData.COMMS_COMPASS.LEVEL_2.A_X_MAX/ 4.0f), 
                  center.y + (sdSysData.COMMS_COMPASS.LEVEL_2.A_Y_MIN / 4.0f));
    p3 = ImVec2(center.x + (sdSysData.COMMS_COMPASS.LEVEL_2.A_X_MAX/ 4.0f), 
                  center.y + (sdSysData.COMMS_COMPASS.LEVEL_2.A_Y_MAX / 4.0f));
    p4 = ImVec2(center.x + (sdSysData.COMMS_COMPASS.LEVEL_2.A_X_MIN/ 4.0f), 
                  center.y + (sdSysData.COMMS_COMPASS.LEVEL_2.A_Y_MAX / 4.0f));
    ...
    */
    ...
    for (size_t pos = 0; pos < sdSysData.COMMS_COMPASS.LEVEL_2.COMPASS_HISTORY.size(); pos++)
    {
      if (sdSysData.COMMS_COMPASS.LEVEL_2.COMPASS_HISTORY.FLAGS[pos].HAS_DATA)
      {
        int color = RAS_GREY;
        if (sdSysData.COMMS_COMPASS.LEVEL_2.COMPASS_HISTORY[pos].X_LOWER_M)
        {
          color = RAS_GREEN;
        }
        else if (sdSysData.COMMS_COMPASS.LEVEL_2.COMPASS_HISTORY[pos].X_UPPER_M)
        {
          color = RAS_YELLOW;
        }
        if (sdSysData.COMMS_COMPASS.LEVEL_2.COMPASS_HISTORY[pos].Y_LOWER_M)
        {
          color = RAS_RED;
        }
        else if (sdSysData.COMMS_COMPASS.LEVEL_2.COMPASS_HISTORY[pos].Y_UPPER_M)
        {
          color = RAS_CYAN;
        }
      ...
      */
      ...
      if (TEST_ALTERTATIVE_COMPASS)    // Display calibration
      {
        ...
        ImGui::Text("BEAR (T): %4.1f  (%4.1f)", 
          sdSysData.COMMS_COMPASS.LEVEL_2.HEADING_DEGREES_TEST, 
          sdSysData.COMMS_COMPASS.bearing() - sdSysData.COMMS_COMPASS.LEVEL_2.HEADING_DEGREES_TEST);
      ...
      if (sdSysData.COMMS_COMPASS.calibrate_on())
      {
        for (int quads = 1; quads < (int)sdSysData.COMMS_COMPASS.LEVEL_2.CALIBRATION_QUADS.size(); quads++)
        {
          ImGui::Text("CAL PT %d | %d | %.0f, %.0f: %.3f", quads,
                                                      (int)sdSysData.COMMS_COMPASS.LEVEL_2.CALIBRATION_QUADS[quads].OFFSET_POINT_LIST.size(), 
