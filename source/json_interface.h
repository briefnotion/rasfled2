...
std::string parse_label(std::string Text, int Pos_Colon);
// Sub out the Label

std::string parse_value(std::string Text, int Pos_Colon);
// Sub out the Value
...
class JSON_ENTRY
{
  ...
  int find_closing(std::string Text, int Start_Pos, char Opening, char Closing);
  // Find matching position of closing character matching opening character.
  //  starting from start position.  e.g. "{{{}}}" "{" "}" returns 5th pos.

  bool check_entry(std::string &Entry, int &Size_Of_Entry, int &Size_Of_Label, 
                              bool &Is_A_Set, bool &Is_A_List, bool &Is_A_Value);
  // Get characteristics of entry.  

  bool parse_item_list(std::string Entry, bool Is_Set, std::string Set_Name);
  // Parse list or set.
  ...
  int find_label_pos(std::string Label_In_List);
  // return pos in JSON entry data list.
  // return -1 if not found.

  int size_of_set(std::string Set_Label);
  // returns the size set list with with Label, within a set.

  bool set_list(std::string Entry);
  // Creates a list within a JSON Entry

  bool set_set(std::string Entry, std::string Set_Name);
  ...
  std::string value_from_list(std::string Label_In_List);
  // Returns the value of label within a JSON list.

  bool get_if_is(std::string Label, int &Value);
  // Returns false if label within a json does not match.
  // Returns true  if label within a json does match and changes.
  //  Value to JSON value.

  bool get_if_is(std::string Label, float &Value);
  // Returns false if label within a json does not match.
  // Returns true  if label within a json does match and changes.
  //  Value to JSON value.

  bool get_if_is(std::string Label, char &Value);
  // Returns false if label within a json does not match.
  // Returns true  if label within a json does match and changes.
  //  Value to JSON value.

  bool get_if_is(std::string Label, bool &Value);
  // Returns false if label within a json does not match.
  // Returns true  if label within a json does match and changes.
  //  Value to JSON value.

  bool get_if_is(std::string Label, CRGB &Value);
  // Returns false if label within a json does not match.
  // Returns true  if label within a json does match and changes.
  //  Value to JSON value.

  bool get_if_is(std::string Label, std::string &Value);
  // Returns false if label within a json does not match.
  // Returns true  if label within a json does match and changes.
  //  Value to JSON value.

  bool get_if_is(std::string Label, STRING_STRING &Value);
  // Returns false if label within a json does not match.
  // Returns true  if label within a json does match and changes.
  //  Value to JSON value.

  bool get_if_is(std::string Label, STRING_BOOL &Value);
  // Returns false if label within a json does not match.
  // Returns true  if label within a json does match and changes.
  //  Value to JSON value.

  bool get_if_is(std::string Label, STRING_INT &Value);
  // Returns false if label within a json does not match.
  // Returns true  if label within a json does match and changes.
  //  Value to JSON value.

  bool get_if_is(std::string Label, STRING_FLOAT &Value);
  // Returns false if label within a json does not match.
  // Returns true  if label within a json does match and changes.
  //  Value to JSON value.

  bool get_if_is(std::string Label, STRING_DOUBLE &Value);
  // Returns false if label within a json does not match.
  // Returns true  if label within a json does match and changes.
  //  Value to JSON value.

  void create_label_value(std::string Label, std::string Value);
  ...
  void put_json_in_set(std::string Set_Name, JSON_ENTRY Entry);
...
class JSON_INTERFACE
{
  ...
  deque<std::string> state_dq_std::string;

  bool ret_success = false;

  CRGB color = sdSysData.get_running_color();

  state_json.ROOT.create_label_value(quotify("red"), quotify(to_std::string((int)(color.r))));
  state_json.ROOT.create_label_value(quotify("green"), quotify(to_std::string((int)(color.g))));
  state_json.ROOT.create_label_value(quotify("blue"), quotify(to_std::string((int)(color.b))));
  state_json.ROOT.create_label_value(quotify("description"), quotify(sdSysData.get_running_color_str()));

  state_json.json_print_build_to_std::string_deque(state_dq_std::string);
  ret_success = deque_std::string_to_file(strFilename, state_dq_std::string, false);
  ...
  To create a more complex list
    ...
    for (size_t pos = 0; pos < COMPASS_HISTORY.size(); pos++)
    {
      if (COMPASS_HISTORY.FLAGS[pos].HAS_DATA && COMPASS_HISTORY.FLAGS[pos].DO_NOT_OVERWRITE)
      {
        ...
        single_point.create_label_value(quotify("X"), quotify(to_std::string(COMPASS_HISTORY[pos].POINT.X)));
        single_point.create_label_value(quotify("Y"), quotify(to_std::string(COMPASS_HISTORY[pos].POINT.Y)));
        single_point.create_label_value(quotify("Z"), quotify(to_std::string(COMPASS_HISTORY[pos].POINT.Z)));
  ...
  To Load:
    ...
    void CAL_LEVEL_3::load_history_and_settings(HMC5883L_PROPERTIES &Props)
    {
      ...
      std::string json_state_file = file_to_std::string(Props.OFFSET_HISTORY_FILE_NAME, ret_success);

      if (ret_success == true)
      {
        ret_success = configuration_json.load_json_from_std::string(json_state_file);
    ...
    ALTERNATIVE METHOD:
      std::string json_state_file = file_to_std::string(strFilename, ret_success);

      if (ret_success == true)
      {
        ret_success = state_json.load_json_from_std::string(json_state_file);
        ...
        if (ret_success == true)
        {
          ...
          color_std::string = red + "," + green + "," + blue;

          color = color.std::stringtoCRGB(color_std::string);

          color_desc = state_json.ROOT.value_from_list("description");

          cons.printw("  Setting running color to CRGB(" + color.CRGBtostd::string() + 
                                                      "), " + color_desc);                                          
  ...
  */
  ...
  void json_debug_to_string_deque(deque<std::string> &JSON_Print_Build, JSON_ENTRY Json_entry, int Level, int Count);

  void json_to_string_deque(deque<std::string> &JSON_Print_Build, JSON_ENTRY Json_entry, int Level, std::string Trailing_Seperator);

  public: 

  JSON_ENTRY ROOT;

  bool load_json_from_string(std::string JSON_Text);
  // boolean failure and success decisions not complete

  void json_debug_build_to_string_deque(deque<std::string> &JSON_Print_Build);
  // Creates an easy to read JSON representation into a string deque.

  void json_print_build_to_string_deque(deque<std::string> &JSON_Print_Build);
