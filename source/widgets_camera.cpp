...
void CAMERA_WIDGET::display(system_data &sdSysData, float Angle)
{
  ...
  if (sdSysData.CAMERA_BACKUP.CAM_VIDEO_AVAILABLE)
  {
    ...
    if (textureID != 0 && !currentFrame.empty()) 
    {
      ...
      if (DISPLAY_PATH)
      {
        ...
        draw_list->AddLine(P0L, P1L, color_green, THICKNESS_1); // Left Line
        draw_list->AddLine(P0R, P1R, color_green, THICKNESS_1); // Right Line
        draw_list->AddLine(P1L, P1R, color_green, THICKNESS_1); // Top Line (Green boundary)


        // --- Draw Segment 2: Yellow Path (Y1 to Y2) ---
        // Draw Left, Right, and Top (Y2). The bottom is already connected by P1L/P1R.
        draw_list->AddLine(P1L, P2L, color_yellow, THICKNESS_2); // Left Line
        draw_list->AddLine(P1R, P2R, color_yellow, THICKNESS_2); // Right Line
        draw_list->AddLine(P2L, P2R, color_yellow, THICKNESS_2); // Top Line (Yellow boundary)

        
        // --- Draw Segment 3: Red Path (Y2 to Y3) ---
        // Draw Left, Right, and Top (Y3). The bottom is already connected by P2L/P2R.
        draw_list->AddLine(P2L, P3L, color_red, THICKNESS_3); // Left Line
        draw_list->AddLine(P2R, P3R, color_red, THICKNESS_3); // Right Line
        draw_list->AddLine(P3L, P3R, color_red, THICKNESS_3); // Top Line (Farthest boundary)
      }
    } 
    else 
    {
  ...
  ImGui::Text("Prc Tme: %2.1f", sdSysData.CAMERA_BACKUP.PROCESSING_TIME);
